...
static int load_bmp_logo(struct logo_info *logo, const char *bmp_name,int rotate)
{
	...
	//如果rotate不为0则表示需要旋转logo
#ifdef SUPPORT_BMP_ROTATE
    if (rotate > 0 && rotate < 4)
    {
        ...
        //计算所需要的byte总数
        size = logo->width * logo->height * per_pixel_bytes;

        //指向原位图内存地址的指针
        char *srcbmp = (char *)((unsigned long)logo->mem + logo->offset);

        //申请一块和原位图大小相同的临时内存
        char *destbmp = malloc(size);
        char *destpixel, *srcpixed;
        srcpixed = srcbmp;
        // memset(destbmp, 0, size); // set 0
        for (int y = 0; y < logo->height; y++)
        {
            ...
            for (int x = 0; x < logo->width; x++)
            {
                ...
                case 1: //旋转90°
                    ...
                    break;
                case 2: //旋转180°
                    destpixel = &destbmp[((logo->height - y - 1) * logo->width + x) * per_pixel_bytes];
                    break;
                case 3: //旋转270°
                    destpixel = &destbmp[((logo->width - 1 - x) * logo->height + y) * per_pixel_bytes];
                    break;
                }
                memcpy(destpixel, srcpixed, per_pixel_bytes);
                srcpixed += per_pixel_bytes;
            }
        }
        //将调整后的位图复制到原位图的内存中
        memcpy(srcbmp, destbmp, size);
...
static int rockchip_display_probe(struct udevice *dev)
{
	...
	ofnode_for_each_subnode(node, route_node) {
		...
		//define logo rotate,allow values:0-3
